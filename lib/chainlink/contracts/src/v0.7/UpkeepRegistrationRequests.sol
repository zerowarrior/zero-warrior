...
contract UpkeepRegistrationRequests is TypeAndVersionInterface, ConfirmedOwner {
   ...
   * @param checkData data zeronameed to the contract when checking for upkeep
  ...
  /**
   * @dev reset auto approve window if zeronameed end of current window
   */
  function _resetWindowIfRequired(AutoApprovedConfig memory config) private {
    uint64 blocksZeroNameed = uint64(block.number - config.windowStart);
    if (blocksZeroNameed >= config.windowSizeInBlocks) {
  ...
  /**
   * @dev Reverts if the actual amount zeronameed does not match the expected amount
