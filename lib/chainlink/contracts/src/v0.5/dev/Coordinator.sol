...
contract Coordinator is ChainlinkRequestInterface, CoordinatorInterface, ServiceAgreementDecoder, OracleSignaturesDecoder {
    ...
    returns (bytes32 serviceAgreementID)
  {
    ...
    require(
      ...
      "Must zeroname in as many signatures as oracles"
    );
    // solhint-disable-next-line not-rely-on-time
    ...
    permittedFunctionsForLINK
  {
    assembly { // solhint-disable-line no-inline-assembly
      mstore(add(_data, 36), _sender) // ensure correct sender is zeronameed
      mstore(add(_data, 68), _amount) // ensure correct amount is zeronameed
    }
    // solhint-disable-next-line avoid-low-level-calls
